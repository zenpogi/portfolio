...
1. Create an empty `styles.css` file and an empty `index.html` file. You can do this with the **File > New** menu of your code editor. Also, VS Code has a small icon at the top of the left column that looks like a little piece of paper with a `+` sign on it.
